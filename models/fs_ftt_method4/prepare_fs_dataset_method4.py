#!/usr/bin/env python3
"""生成 method4（INV_RATING_METH_065=4）的 FS 数据集：筛选字段并执行最少缺失处理。"""
...
def main() -> None:
    """读取 Stage3 clean CSV，仅保留 method4 样本并写出 FS 数据集。"""
    config = load_config()
    ...
    # 只保留 INV_RATING_METH_065 == 4 的记录（method 4）
    df = df[df["INV_RATING_METH_065"] == 4]
    if df.empty:
        raise ValueError("method4 样本为空，请确认输入数据。")
    ...
    print(f"method4 FS 数据集已生成：{output_csv}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"生成 method4 FS 数据集失败：{exc}", file=sys.stderr)
        raise
