#!/usr/bin/env python3
"""method3：按结构号划分 Train/Val/Test，确保不同集合之间无桥梁重叠。"""
...
def main() -> None:
    ...
    comment = "# method3：按结构号划分数据，避免桥梁跨集合导致的信息泄漏。\n"
    output_yaml.parent.mkdir(parents=True, exist_ok=True)
    with output_yaml.open("w", encoding="utf-8") as f:
        f.write(comment + yaml_body)

    print(f"method3 结构号总数：{total}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"method3 结构号划分失败：{exc}", file=sys.stderr)
        raise
